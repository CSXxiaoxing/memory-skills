...
name: memory-skills
description: |
  基于人脑记忆机制的项目级AI持久化记忆技能，模拟记忆的创建、存储、检索、压缩和遗忘过程。

  **核心特性**：每个项目有独立的项目级记忆存储，项目之间不混淆。

  **必须使用此技能**，当用户提到以下任何情况时：
  - "记住"、"记忆"、"记一下"、"帮我存一下"
  - "之前"、"上次"、"曾经"、"过去"、"之前那个项目的"
  - "查找记忆"、"搜索记忆"、"找一下之前"
  - "压缩记忆"、"整理记忆"
  - "加载大脑"、"查看记忆库"
  - "导出记忆"、"备份记忆"
  - 涉及当前项目或指定项目的之前解决方案、设计方案、代码实现
  - 需要调用记忆系统的任何操作
  - "代码变更"、"追踪修改"、"自动记忆"、"安装hook"
  ...
  **核心能力**：
  ...
  - 创建结构化记忆文档（YAML+Markdown格式）
  - 智能语义检索（不仅仅关键词匹配）
  - LLM语义压缩（保留代码块和核心信息）
  - Python不可用时自动降级为LLM直接处理
  - **类脑记忆系统**：4种记忆类型（工作/情景/语义/程序）分级存储
  - **节点追踪**：每个任务节点完成后自动记录
  - **遗忘机制**：冲突遗忘 + 时间遗忘 + LLM自主决策遗忘
...
**检测当前项目**：按以下顺序检测项目根目录：
1. `.memory/brain.md` 存在 → 使用该项目
2. `.git` 存在 → 使用 Git 仓库根目录
3. `package.json` / `Cargo.toml` / `go.mod` 存在 → 使用项目根目录
4. 使用当前工作目录
...
```bash
# 检测当前项目
# 1. 查找 .memory/brain.md 所在目录
# 2. 或查找 .git 所在目录
# 3. 或查找 package.json/Cargo.toml/go.mod 所在目录
# 4. 或使用当前工作目录
...
**🔍 项目检测规则**：
1. 向上查找 `.memory/brain.md` → 使用该目录作为项目根
2. 向上查找 `.git` → 使用 Git 仓库根目录
3. 向上查找项目配置文件 → 使用项目根目录
4. 使用当前工作目录
...
**记忆决策**：
- `quality_score >= 70`：创建新记忆
- `50 <= quality_score < 70`：压缩后创建
